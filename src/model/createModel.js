...
module.exports = function createModel(name, module) {
  const stubFile = path.resolve(__dirname, '../../stub/models/Module.js');

  const modelFileName = `${name}.js`;
  const targetFolder = path.resolve(process.cwd(), 'Modules', module, 'Models');
  const destFile = path.join(targetFolder, modelFileName);
  ...
  if (fs.existsSync(destFile)) {
    console.log(`⚠️ Model '${modelFileName}' already exists in module '${module}'.`);
    return;
  }
  ...
  console.log(`✅ Model '${modelFileName}' created in module '${module}'`);
};